...
def get_minibatch(roidb, num_classes):
    ...
    roi_n_blob = np.zeros((0, 5), dtype=np.float32)
    ...
    for i_im in xrange(num_images):
        ...
        roi_n_blob_this_image = np.hstack((batch_ind, roi_n))
        roi_n_blob = np.vstack((roi_n_blob, roi_n_blob_this_image))
    ...
    blobs['roi'] = roi_blob
    blobs['roi_normalized'] = roi_n_blob
    if cfg.CONTEXT:
