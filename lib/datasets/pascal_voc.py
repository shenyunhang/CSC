...
import subprocess
import uuid
from voc_eval import voc_eval, voc_eval_corloc
...
class pascal_voc(imdb):
    ...
    def pseudo_gt_roidb(self):
        ...
        gt_roidb = []
        for im_i in range(self.num_images):
            if im_i % 1000 == 0:
            ...
            for cls in range(1, self.num_classes):
                bbox = roidb[cls - 1][im_i]
                num_objs = num_objs + bbox.shape[0]
    ...
    def edge_boxes_roidb(self):
        ...
        roidb = self._load_edge_boxes_roidb()
    ...
    def _load_edge_boxes_roidb(self):
    ...
    def mcg_roidb(self):
        ...
        if cfg.WSL and not cfg.USE_PSEUDO:
            roidb = self._load_mcg_roidb()
        elif not cfg.WSL and cfg.USE_PSEUDO:
            pseudo_gt_roidb = self.pseudo_gt_roidb()
            mcg_roidb = self._load_mcg_roidb()
            roidb = imdb.merge_roidbs(pseudo_gt_roidb, mcg_roidb)
        elif not cfg.WSL and not cfg.USE_PSEUDO:
            gt_roidb = self._gt_roidb()
            mcg_roidb = self._load_mcg_roidb()
            roidb = imdb.merge_roidbs(gt_roidb, mcg_roidb)
        else:
            raise Exception('WSL and USE_PSEUDO should not happen together.')
